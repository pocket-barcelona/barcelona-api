...
export class PlacesService {
	...
	static getById = async (placeId: PlaceDocument['placeId']): Promise<PlaceDocument | null> =>
		getByIdHandler(placeId);

	static getRelatedPlaces = async (
		params: ReadPlaceInput['params']
	): Promise<ScanResponse<PlaceDocument> | null> => getRelatedPlacesHandler(params);

	static getPlaceCategories = async (): Promise<ScanResponse<CategoryDocument> | null> =>
		getPlaceCategoriesHandler();

	static getSearchPrepopulate = async (): Promise<ScanResponse<PlaceSearchDocument> | null> =>
		getSearchPrepopulateHandler();

	/** This is for the Dashboard which needs a list of places and their IDs */
	static getPlaceLookupList = async (): Promise<ScanResponse<PlaceLookupDocument> | null> =>
		getPlaceLookupHandler();
