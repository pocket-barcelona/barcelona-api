...
import { getListHandler, getTagsHandler } from './functions/index.js';
...
export class PoiService {
	static getList = async (
		...
		const isFilteringTags =
			filters.tagId && Array.isArray(filters.tagId) && filters.tagId.length > 0;
	...
	static getTags = async (): Promise<ScanResponse<PoiTagDocument> | null> => getTagsHandler();
}
