...
export class PlanHelper {
  ...
  buildPlanResponse(
    ...
    // @todo...
    const numberOfPlaces = itinerary.flatMap(d => d.places.length).reduce((accumulator, currentValue) => {
      return accumulator + currentValue;
    });

    const priceAverage = 0; // might not need this
    const includesPlacesOutsideCity = itinerary.flatMap(d => d.places.map(p => p.metroZone === -1 || p.metroZone === -2)).some(p => p);
    const timeOfDay = 1; // @todo
    const centralBarriosOnly = itinerary.flatMap(d => d.places.map(p => [...CENTRAL_BARRIO_IDS].includes(p.barrioId))).every(p => p);
   ...
   */
  getPlanTitle(
    themeName: StructuredPlanDayProfile["name"],
    results: PlaceDocument[]
  ): string {
  ...
  private getDayByDayItinerary(
    ...
    while (allPlaces.length > 0 && itinerary.length < numberOfDays) {
      ...
      const sliceAt =
        placeIndex > DOCUMENT_LIST_RETURN_LIMIT
          ? DOCUMENT_LIST_RETURN_LIMIT
          : placeIndex;
      const thisDayPlaces = allPlaces.splice(0, sliceAt);

      itinerary.push({
        action:
          theme.verbs && theme.verbs.length > 0 ? theme.verbs[0] : "Go to",
        dayNumber: dayNumber,
  ...
  getThemeInputParams = (theme: StructuredPlanDayProfile): ThemeInputSpecs => {
    ...
    const hasExcludePlaceIds = theme.placeIdsExclude
      ? theme.placeIdsExclude.length > 0
      : false;
    ...
    const hasCategoryIds = theme.categoryIds
      ? theme.categoryIds.length > 0
      : false;
    ...
    const hasFreeToVisit =
      theme.freeToVisit !== undefined ? theme.freeToVisit : null;
    ...
    const hasTimeRecommendedOptions =
      Array.isArray(theme.timeRecommendedOptions) &&
      theme.timeRecommendedOptions.length > 0; // more checks?
    const hasRequiresBookingOptions =
      Array.isArray(theme.requiresBookingOptions) &&
      theme.requiresBookingOptions.length > 0; // more checks?
   ...
   */
  private orderResultsBasedOnLatLng = (
    places: PlaceDocument[]
  ): PlaceDocument[] => {
