	...
	switch (tokenValidity) {
		...
		case CheckResetTokenEnum.NoDocument: {
			return res
				...
				.json(error('Email or token invalid. Please reset your password again', res.statusCode));
		}
		...
		case CheckResetTokenEnum.TokenExpired: {
			...
			return res.status(StatusCodes.NOT_FOUND).json(error('Token expired', res.statusCode));
		}
